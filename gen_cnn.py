...
def main():
    ...
    ld = DataLoader(raw_datasets["test"], batch_size=training_args.per_device_eval_batch_size)

    labels = raw_datasets["test"]['highlights']
    preds = []
    
    safe_length = block_size - 1 - model_args.ztokens - 128 # sep*1, ztokens, summary
    cachep = "/yinyongjing/junran/cgae/cnn_pred.pkl"
    if not os.path.exists(cachep):
        for idx, cur in tqdm(enumerate(ld), desc = "generating"):
            ...
            appseq = [tokenizer.sep_token + special_seq] * len(x)
            appxx = tokenizer(
                appseq,
                return_tensors="pt"
            )
            ...
            res = tokenizer.batch_decode(pred[:,input_ids.size(1):], skip_special_tokens=True, clean_up_tokenization_spaces=True)

            preds += res

        with open(cachep, "wb") as f:
            print("Saved cached preds.")
            pickle.dump(preds, f)
    else:

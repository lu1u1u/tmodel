...
def main():
    ...
    def safe_encode(srclist, tarlist, tail_trunc_length = 512):
        ...
        for src, tar in zip(srclist, tarlist):
            ...
            enctail = tokenizer.sep_token + tar + tokenizer.eos_token
            enctail_ids = tokenizer.encode(enctail)
            if len(enctail_ids) >= block_size // 1.5: # ill tail
                enctail_ids = enctail_ids[:tail_trunc_length] 
            enchead = tokenizer.bos_token + src 
            enchead_ids = tokenizer.encode(enchead)[:block_size-len(enctail_ids)]
            encres_app = enchead_ids + enctail_ids + [tokenizer.pad_token_id] * max(block_size-len(enchead_ids)-len(enctail_ids), 0)
            encres_atm = [1 if  i != tokenizer.pad_token_id else 0 for i in encres_app]
            assert len(encres_app) == len(encres_atm) == block_size
            encres["input_ids"].append(encres_app)
            encres["attention_mask"].append(encres_atm)
            #print(encres, len(encres_app), len((encres_atm)))
            ...
            dechead = tokenizer.bos_token + src 
            dechead_ids = tokenizer.encode(dechead)[:block_size-len(dectail_ids)]
            decres_app = dechead_ids + dectail_ids + [tokenizer.pad_token_id] * max(block_size-len(dechead_ids)-len(dectail_ids), 0)
            decres_atm = [1 if  i != tokenizer.pad_token_id else 0 for i in decres_app]
    ...
    def tokenize_function(examples):
        with CaptureLogger(tok_logger) as cl:
            ...
            encres, decres = safe_encode(srclist = examples['article'], tarlist = examples['highlights'])
            ...
            for i in range(pos_mask.size(0)):
                try:
                    ...
                    with open("./cnn_wrongs.txt", "a") as f:
                        f.write("==========================\n")
